...
//        accessToken: '13543390771',
	pwd: "5221efa5551b4a08c2bd4294b39b002d",
...
//            判断聊天窗口是否打开
		if($('#draggable').css('display')=='none'){
			//声音提示
			$('#toneBox').html('<audio autoplay="autoplay" id="tone"><source src="audio/4.mp3" type="audio/mpeg" /></audio>');
			msgCount++;
			if(msgCount>99){
				msgCount==99;
			}
			$('#newsCount').find('i').text(msgCount).css('display','block');
		}
...
//                console.log(data);
			...
			$(style).children('.msgContainer').css({'display':'flex','justify-content':'flex-end'});
			$(style).children('.msgContainer').children('.msgRight').css({'margin-right':'42px','margin-left':'0','display':'flex','justify-content':'flex-end','flex-wrap':'wrap'});
			$(style).find('.msgText').css({'background-color':'#D9EFFF'});
			$(style).find('.qipao').attr('src','Public/chat2/images/chat2.png');
			$(style).find('.qipao').css({'right':'-7px','left':'auto'});
			$(style).find('.msgLeft').css({'right':'4px','left':'auto'});
...
$(function() {
	$( "#draggable" )
		.draggable({ handle: ".chatHeader",containment: "parent"})
		.resizable({
			maxHeight: 2048,
			maxWidth: 2048,
			minHeight: 400,
