...
function del_fun_spec(del_url)
{
	if(confirm("确定要删除吗?这将导致某些使用该规格的商品出现规格缺失，若继续执行，请先将商品点击不显示，再进行删除，最后记得把商品的规格补全哦！"))
		location.href = del_url;
}
...
function changeTableVal2(table,id_name,id_value,field,obj)
{
    ...
    $.ajax({
        url:"/Store/Index/changeTableVal?table="+table+"&id_name="+id_name+"&id_value="+id_value+"&field="+field+'&value='+value,
        success: function(data){
            $(obj).attr('src',src);
        }
    });
