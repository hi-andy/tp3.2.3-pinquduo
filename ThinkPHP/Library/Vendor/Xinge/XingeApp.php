...
class XingeApp {
	...
	const IOSENV_PROD = 1;//向iOS设备推送时必填，1表示推送生产环境；2表示推送开发环境。推送Android平台不填或填0
	const IOSENV_DEV = 2;
	 ...
	 */
	public static function PushAllAndroid($accessId, $secretKey, $title, $content,$custom = array('type'=>'messgae', 'typeid'=>'0'))
	{
		$push = new XingeApp($accessId, $secretKey);
		$mess = new Message();
		$mess->setTitle($title);
...
//		$mess->setCustom($custom);
	 ...
	 * 使用默认设置推送消息给所有设备ios版
	 */
	public static function PushAllIos($accessId, $secretKey, $content, $environment,$custom = array('type'=>'messgae', 'typeid'=>'0'))
	{
		$push = new XingeApp($accessId, $secretKey);
		$mess = new MessageIOS();
		$mess->setAlert($content);
...
class ClickAction {
	...
	const TYPE_ACTIVITY = 1; // 动作类型，1打开activity或app本身，2打开浏览器，3打开Intent
	const TYPE_URL = 2;
	const TYPE_INTENT = 3;
}
