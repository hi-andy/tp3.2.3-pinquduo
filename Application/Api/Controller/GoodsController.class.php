...
//		$type = I('type');//0->不是海淘的  1->是海淘的
   ...
   //商品特殊类型 1-海淘，2-限时秒杀，3-一元夺宝，4-99专场，5-多人拼团
	...
	function getsearch()
	{
		...
		if ($result) {
            M()->execute("UPDATE tp_keyword_search SET `update_time` = ".time().", increase = increase + 1 WHERE keyword='$key'");
        } else {
            M()->execute("INSERT INTO tp_keyword_search SET `create_time` = ".time().", increase = 1, `update_time` = ".time().", keyword='$key'");
        }
