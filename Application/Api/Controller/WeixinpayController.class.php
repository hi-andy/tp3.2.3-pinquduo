    ...
    function getJSAPI($order, $time_expire=1800){
        ...
        if($order['prom_id']){
            ...
            if(I('wxv') != 2) {
                ...
                $go_url =C('SHARE_URL').'/pinquduowx-test/order_detail.html?order_id='.$prom_info['order_id'].'&type='.$type.'&user_id='.$order['user_id'];
            }
        }else{
            if(I('wxv') != 2) {
                $go_url =C('SHARE_URL').'/order_detail.html?order_id='.$order['order_id'].'&type=2&user_id='.$order['user_id'];
            } else {
                $go_url =C('SHARE_URL').'/pinquduowx-test/order_detail.html?order_id='.$order['order_id'].'&type=2&user_id='.$order['user_id'];
            }
        }
        if(I('wxv') != 2) {
            $back_url = C('SHARE_URL')."/goods_detail.html?goods_id={$order['goods_id']}";
        } else {
            $back_url = C('SHARE_URL')."/pinquduowx-test/goods_detail.html?goods_id={$order['goods_id']}";
        }
        //①、获取用户openid
        $tools = new \JsApiPay();
