...
class GoodsnewController extends BaseController {
    ...
    public function _initialize() {
		...
		if(ACTION_NAME == "addEditGoods" && $_GET['is_ajax']!=1){
			$goods_id = (int)I('id');
			$store_id = (int)I('storeId');
			if($goods_id<=0 || $store_id<=0){
				die('error');
			}
			$good_info = M('goods')->where("goods_id={$goods_id}")->find();
			if($good_info['store_id']!=$store_id){
				die('error storeid');
			}
			$_SESSION['merchant_id'] = I('storeId');					
		}
