...
//					session('merchant_name',$merchant_info['store_name']);
	...
	{
		if($store['id'] != $_COOKIE['storeid']) {
		setcookie("merchant_id",$store['id'],time()+24*3600);
		setcookie("trade_no",$type['trade_no'],time()+24*3600);
		setcookie("is_haitao",$type['is_haitao'],time()+24*3600);
		setcookie("state",$store['state'],time()+24*3600);
		setcookie("merchant_name",$store['merchant_name'],time()+24*3600);
		}
	}
