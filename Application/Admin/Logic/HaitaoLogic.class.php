 ...
 * @package Home\Logic
 */
class HaitaoLogic extends RelationModel
{
    ...
    public function goods_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
                ...
                $sql = "SELECT * FROM  __PREFIX__haitao WHERE `is_show`=1 ORDER BY parent_id ASC";
                $goods_category = D('haitao')->query($sql);
    ...
    {
        global $goods_label, $goods_label2;
        $sql = "SELECT * FROM  __PREFIX__goods_label where `is_show` = 1 ORDER BY parent_id  ASC";
        $goods_label = D('goods_label')->query($sql);
        $goods_label = convert_arr_key($goods_label, 'id');
        foreach ($goods_label AS $key => $value)
        {
            if($value['level'] == 1)
                $this->get_label_tree($value['id']);
        }
        /*
		foreach ($goods_category2 AS $key => $value)
		{
    ...
    public function get_cat_tree($id)
{
    ...
    /**
     * 获取指定id下的 所有标签
     * @global type $goods_label 所有商品标签
     * @param type $id 当前显示的 菜单id
     * @return 返回数组 Description
     */
    public function get_label_tree($id)
    {
        global $goods_label, $goods_label2;
        $goods_label2[$id] = $goods_label[$id];
        foreach ($goods_label AS $key => $value){
            if($value['parent_id'] == $id)
            {
                $this->get_label_tree($value['id']);
                $goods_label2[$id]['have_son'] = 1; // 还有下级
            }
        }
    }
     ...
     * @return 返回数组 Description
     */
    public function refresh_label_cat($id)
    {
        $GoodsLabel = M("Goods_label"); // 实例化User对象
        $cat = $GoodsLabel->where("id = $id")->find(); // 找出他自己
        if($cat['print'] == 1)
        {
             echo '666';
        }
    ...
    function find_parent_cat($cat_id)
    {
        ...
        $cat_list =  M('haitao')->getField('id,parent_id,level');
     ...
     * @param type $cat_id
     */
    function find_parent_label($cat_id)
    {
        if($cat_id == null)
            return array();

        $cat_list =  M('goods_label')->getField('id,parent_id,level');
        $cat_level_arr[$cat_list[$cat_id]['level']] = $cat_id;
...
//        var_dump(M()->getLastSql());
    ...
    {
        $labellist = M('goods_label')->where('is_show = 1 and parent_id = 0') -> select();
        for($i = 0;$i<count($labellist);$i++)
        {
            $children = M('goods_label')->where('is_show = 1 and parent_id = '.$labellist[$i]['id'])->select();
            for($j = 0;$j<count($children);$j++)
            {
                $temp = $children[$j]['name'];
                $children[$j]['name'] = '--'.$temp;
            }
            $labellist[$i]['children'] = $children;
        }
//       print_r($labellist);die;
        return $labellist;
    }
}