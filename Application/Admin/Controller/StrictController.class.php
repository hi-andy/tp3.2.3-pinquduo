...
class StrictController extends BaseController {
        ...
        if($_POST) {
            ...
            if ($res) {
                $return_arr = array(
                    'status' => 1,
                    'msg' => '添加成功',
                    'data' => array('url' => U('Admin/Strict/SpeciallyList')),
                );
                ...
                $return_arr = array(
                    'status' => -1,
                    'msg' => '添加失败',
                    'data' => array('url' => U('Admin/Strict/addSpecially')),
                );
                $this->ajaxReturn(json_encode($return_arr));
            }
        }
        ...
        if($_POST['type']>0)
        {
            ...
            if($res)
            {
                ...
                $return_arr = array(
                    'status' => -1,
                    'msg'   => '修改失败',
                    'data'  => array('url'=>U('Admin/Strict/EditSpecially')),
                );
                $this->ajaxReturn(json_encode($return_arr));
            }
        }
    ...
    {
        $where = 'show_type=0 and is_special=9 and is_audit=1';
        ...
        if(($_GET['is_ajax'] == 1) && IS_POST)
        {
            ...
            if(!$Goods->create(NULL,$type))// 根据表单提交的POST数据创建数据对象
            {
                ...
                $return_arr = array(
                    'status' => 1,
                    'msg'   => '操作成功',
                    'data'  => array('url'=>U('Admin/Strict/Goodsindex')),
                );
                $this->ajaxReturn(json_encode($return_arr));
            }
        }
...
//		$brandList = $GoodsLogic->getSortBrands();
    ...
    {
        $id = I('id');
        $count = M('goods')->where('`is_special`=9 and `exclusive_cat`='.$id)->count();
