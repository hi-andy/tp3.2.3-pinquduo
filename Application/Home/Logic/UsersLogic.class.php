...
class UsersLogic extends RelationModel
{
     ...
     * 第三方登录
     */
    public function thirdLogin($parameters=array())
    {
        //　第三方传递参数
        $openid = $parameters['openid']; //第三方返回唯一标识
        $oauth = $parameters['oauth']; //来源
        ...
        if($oauth && ($unionid || $openid)) {
            ...
            //　调试信息
            // M('admin_log')->data(array('admin_id'=>$user['user_id'],'log_ip'=>'127.0.0.1','log_info'=>'thirdLogin','log_time'=>time(),'log_url'=>json_encode($user)))->add();
            if($user['user_id'] > 0){
                ...
                $userData['head_pic'] = CDN . "/" . $qiniu_result[0]["key"];
                $userData['status'] = 1;
...
//        $type = I('get.type',0);
    ...
    public function cancel_order($user_id,$order_id){
        ...
        $row = M('order')->where(array('order_id'=>$order_id,'user_id'=>$user_id))->save(array('order_status'=>3,'is_cancel'=>1,'order_type'=>5));
        if($order['prom_id']!=null) {
            $res = M('group_buy')->where(array('order_id' => $order_id, 'user_id' => $user_id))->save(array('is_cancel' => 1));
        }else{
            $res =1;
        }
        if(!$row || !$res) {
    ...
    /*
     * 当订单取消的时候将库存，销量，门店销量，使用的优惠卷回复到原来的样子
     * */
    public function fallback($order)
    {
        //商品销量减去订单中的数量
        M('goods')->where('`goods_id`='.$order['goods_id'])->setInc('store_count',$order['num']);
        $is_special =  M('goods')->where('`goods_id`='.$order['goods_id'])->getField('is_special');
        if($is_special==7){
            M('goods_activity')->where('`goods_id` = '.$order['goods_id'])->setInc('quantity',$order['num']);
        }
        //规格库存回复到原来的样子
        ...
        {
            M('coupon')->where('`id`='.$order['coupon_id'])->setDec('use_num');
            //把优惠卷还给用户
            $data['use_time'] = 0;
            $data['is_use'] = 0;
            $data['order_id'] = 0;
            M('coupon_list')->where('`id`='.$order['coupon_list_id'])->data($data)->save();
        }
    }
     ...
     * @return bool
     */
    public function sms_code_verify($mobile,$code){
        //判断是否存在验证码
        $data = M('sms_log')->where(array('mobile'=>$mobile,'code'=>$code))->order('id DESC')->find();
        if(empty($data))
        ...
        M('sms_log')->where(array('mobile'=>$mobile,'code'=>$code))->delete();
