...
//	}
	...
	function workbench(){
		...
		$info[0]['key'] = '今日销售额';
		$info[0]['value'] = $Order->where('pay_status=1 and add_time>'.$today.' and add_time<'.($today+24*3600).' and store_id = '.$store_id.' and order_type not in(9,11,13)')->sum('order_amount');
		empty($info[0]['value']) &&  $info[0]['value']=0;
		$info[1]['key'] = '今日订单数';
		$info[1]['value'] = $Order->where('pay_status=1 and add_time>'.$today.' and add_time<'.($today+24*3600).' and store_id = '.$store_id.' and order_type not in(9,11,3)')->count();
