        ...
        for ($i=0;$i<$nums;$i++){
            ...
            if (!empty($all[$i]['prom_id'])) {
                ...
                if(!empty($all[$i]['goodsInfo'])){
                    ...
                    // 修改商品列表的商品主图和商品列表图，主要是兼容新旧商家后台版本造成的图片问题
                    // 这个是2_0_1的老接口，后面要调用新的接口  温立涛  2017-09-05 16:20
                    $imgArray = getimagesize($all[$i]['goodsInfo']['original_img']);
                    if((int)$imgArray[0] == (int)$imgArray[1]){
                        $temp = $all[$i]['goodsInfo']['original'];
                        $all[$i]['goodsInfo']['original'] = $all[$i]['goodsInfo']['original_img'];  //正方形
                        $all[$i]['goodsInfo']['original_img'] = $temp; 	      //长方形
                    }
