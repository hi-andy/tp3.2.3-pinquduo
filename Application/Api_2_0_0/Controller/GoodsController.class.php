	...
	function getOtheyMore($id,$type,$page,$pagesize,$version,$ajax_get)
	{
		if($type==0)
		{
			...
			if(in_array("$id", $ids[0]))
			{
				...
				if(in_array("$id", $ids2[0]))//确定为二级分类id
				{
					...
					if($version=='2.0.0'){
						$data = $this->getGoodsList($condition2,$page,$pagesize,null);
					}else {
					...
					if($version=='2.0.0'){
						...
						$data = $this->getGoodsList($where,$page,$pagesize,null);
					}else {
		...
		elseif($type==1)
		{
			...
			else
			{
				...
				if($version=='2.0.0'){
					$data = $this->getGoodsList($condition,$page,$pagesize,null);
				}else{
