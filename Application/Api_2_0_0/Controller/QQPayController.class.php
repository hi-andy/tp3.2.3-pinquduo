...
class QQPayController extends BaseController
{
	...
	//if(empty($order)){$order=array('order_sn' => date('YmdHis').mt_rand(1000,9999), 'order_amount' => 0.1);}
        ...
        if($order['prom_id']){
            ...
            $go_url ='http://wx.pinquduo.cn/pinquduo-test/order_detail.html?order_id='.$prom_info['order_id'].'&type='.$type.'&user_id='.$order['user_id'];
        }else{
            $go_url ='http://wx.pinquduo.cn/pinquduo-test/order_detail.html?order_id='.$order['order_id'].'&type=2&user_id='.$order['user_id'];;
        }
	$data['go_url'] = $go_url;
