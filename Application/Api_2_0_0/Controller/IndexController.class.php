    ...
    public function getHaiTao()
    {
        ...
        if(empty(redis($rdsname))) {//判断是否有缓存
            ...
            if($version=='2.0.0'){
                ...
                $total = M('goods')->where('`show_type`=0 and is_special=1 and `is_on_sale`=1 and is_audit=1 and `is_show`=1 and haitao_cat != 65')->count();
                $goods = M('goods')->where('is_special = 1 and is_show= 1 and is_audit= 1 and is_on_sale = 1 and  shpw_type = 0 and category_id != 65')->field('goods_id,goods_name,original_img,shop_price,market_price,prom,prom_price,free')->page($page, $pagesize)->order('is_recommend desc,sort asc')->select();
                foreach ($goods as &$v) {
