...
//        $this->encryption();
    ...
    function getJiuJiu()
    {
        ...
        $pagesize = I('pagesize', 20);
        $version = I('version');
        $rdsname = "getJiuJiu".$version.$page.$pagesize.$version;
        if(empty(redis($rdsname))) {//判断是否有缓存
    ...
    function get_Seconds_Kill()
    {
        ...
        $page = I('page',1);
        $pagesize = I('pagesize',20);
        $rdsname = "get_Seconds_Kill".$version.$starttime.$page.$version;
	...
	* */
    ...
    function get_Free_Order()
    {
        ...
        $version = I('version');
        I('ajax_get') &&  $ajax_get = I('ajax_get');//网页端获取数据标示
        $rdsname = "get_Free_Order".$version.$free_num.$page.$pagesize;
