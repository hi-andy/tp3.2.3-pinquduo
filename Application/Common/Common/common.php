...
function get_user_info($user_id_or_name,$type = 0,$oauth='',$unionid='')
{
    ...
    if ($userInfo = redis($redisKey)) {
         ...
         * 完成旧用户数据合并后删除。
         */
        $time = strtotime('2017-8-24'); //　以下代码发布日期,检查此日期之前的注册用户记录
        if ($userInfo && $userInfo['reg_time'] < $time) {
...
function goods_thum_images($goods_id,$width,$height){
    ...
    if (strstr($original_img, CDN) !== false) {
        ...
        $goods_thumb_name = "goods_thumb_{$goods_id}_{$width}_{$height}_" . $img1[0];

        // 这个商品 已经生成过这个比例的图片就直接返回了
        if (file_exists($path . $goods_thumb_name . '.jpg')) return C('HTTP_URL').'/' . $path . $goods_thumb_name . '.jpg';
        if (file_exists($path . $goods_thumb_name . '.jpeg')) return C('HTTP_URL').'/' . $path . $goods_thumb_name . '.jpeg';
        if (file_exists($path . $goods_thumb_name . '.gif')) return C('HTTP_URL').'/' . $path . $goods_thumb_name . '.gif';
        if (file_exists($path . $goods_thumb_name . '.png')) return C('HTTP_URL').'/' . $path . $goods_thumb_name . '.png';
        ...
        */
        ...
        return C('HTTP_URL').'/' . $path . $goods_thumb_name;
    }
}
...
function getCatGrandson ($cat_id)
{
    ...
    // 把整张表找出来
    $GLOBALS['category_id_arr'] = M('haitao')->getField('id,parent_id');
    // 先把所有儿子找出来
    $son_id_arr = M('haitao')->where("parent_id = $cat_id")->getField('id',true);
 ...
 * author Fox
 */
function SendXinge($message,$account,$custom = array('type'=>'message', 'typeid'=>'0')){

    vendor('Xinge.XingeApp');
    $IOS_Xinge = new \XingeApp(C('Xinge')['IOS_ACCESSID'],C('Xinge')['IOS_SECRETKEY']);
    $ios_ret = '';
    //发送ios多个消息
    $mess = new MessageIOS();
    $mess->setExpireTime(86400);
    $mess->setAlert($message);
    $mess->setBadge(1);
...
//	if(is_array($account)){
//		//IOS推送多个信息
//		$ios_ret = $IOS_Xinge->PushSingleAccount(4, $account, $mess, XingeApp::IOSENV_DEV);
//	}else{
//		//IOS推送单个消息
    $ios_ret = $IOS_Xinge->PushSingleAccount(0, $account, $mess, XingeApp::IOSENV_PROD);
//	}
    ...
    $action->setActivity('com.jumper.spellgroup.ui.detail.DetailPromActivity');

    $action ->setAtyAttrPendingIntentFlag(3);


    $mess->setAction($action);

    //安卓推送单个消息
    $Android_Xinge = new \XingeApp(C('Xinge')['AD_ACCESSID'],C('Xinge')['AD_SECRETKEY']);
    //判断是否是数组
    if(is_array($account)) {
        $android_ret = $Android_Xinge->PushAccountList(0, $account, $mess);
    }else{
        $android_ret = $Android_Xinge->PushSingleAccount(3, $account, $mess);
    }

//     echo json_encode($android_ret);

    return json_encode( array('ios'=>$ios_ret,'android'=>$android_ret));
}
 ...
 * author Fox
 */
function SendAllXinge($message,$custom = array('type'=>'message', 'typeid'=>'0')){

    vendor('Xinge.XingeApp');

    $Xinge = new \XingeApp();

    $android_ret = $Xinge->PushAllAndroid(C('Xinge')['AD_ACCESSID'],C('Xinge')['AD_SECRETKEY'],$message,XingeApp::IOSENV_DEV,$custom);

    $ios_ret = $Xinge->PushAllIos(C('Xinge')['IOS_ACCESSID'],C('Xinge')['IOS_SECRETKEY'],$message,XingeApp::IOSENV_DEV,$custom);
