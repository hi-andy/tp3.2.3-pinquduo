...
class OrderLogic extends RelationModel
{
    ...
    public function getOrderList($condition,$order='',$start=0,$page_size=20){
        $res = M('order')->where($condition)->where('`prom_id` is null')->limit("$start,$page_size")->order($order)->select();
        return $res;
    }
    /*
     * 获取订单商品详情
...
//        $res[0]['address2'] = $this->getAddressName($res[0]['province'],$res[0]['city'],$res[0]['district']);
    ...
    public function orderActionLog($order_id,$action,$note=''){    	
		...
		$data['order_status'] = $order['order_type'];
...
//			$this->changeOrdeStatus($out_trade_no);
	...
	public function  alipayBackPay($out_trade_no,$refund_fee){
		require("plugins/payment/alipay/aop/AopClient.php");
		...
		require("plugins/payment/alipay/aop/request/AlipayTradeRefundRequest.php");
	...
	public function changeOrdeStatus($order_sn)
{
	$data['order_status'] = 5;
	$data['order_type'] = 7;
	M('order')->where('`order_sn`='.$order_sn)->data($data)->save();
	$data2['one_time'] =  $data2['two_time'] = $data2['ok_time'] = time();
	M('return_goods')->where('`order_sn`='.$order_sn)->data($data2)->save();
}
...
//		$data['admin_id'] = session('admin_id');商户里面的操作，没有用户
