...
class ChatController extends BaseController
{
    ...
    public function get_chat($to='', $from='', $chat_type='', $page=0, $pagesize=20){
        if ($to && $from && $chat_type) {
            ...
            $result = M('chat')->where($where)->order('timestamp desc')->limit($page,$pagesize)->select();
            foreach ($result as $key => $value){
                ...
                $data[$key]['payload'] = htmlspecialchars_decode($value['payload']);
...
//            if (empty(redis('get_unread'))) {
                ...
                foreach ($data1 as $k1 => $v1) {
                    $data[$k1] = $v1;
                    $payload = M('chat')->where(array('froms' => $v1['froms'], 'tos' => $user_id))->order('timestamp desc')->getField('payload');
                ...
                foreach ($data2 as $k2 => $v2) {
                    ...
                    $payload = M('chat')->where(array('froms' => $v2['froms'], 'tos' => $user_id))->order('timestamp desc')->getField('payload');
