...
class AutomationController extends BaseController
{
    ...
    public function auto_add_group_buy() {
        ...
        foreach ($goods as $k => $v){
            ...
            $group_buy_values .= "(".time().",{$end_time},{$v['goods_id']},{$v['prom']},1,{$v['prom_price']},'{$v['goods_name']}',{$v['market_price']},'{$v['goods_name']}','".CDN."/Public/upload/logo/logo.jpg',{$user['user_id']},{$v['store_id']},1),";
        }
        $ids = substr($ids, 0, -1);
        $group_buy_values = substr($group_buy_values, 0, -1);
        $sql_group_buy = "INSERT INTO tp_group_buy(start_time,end_time,goods_id,goods_num,order_num,price,intro,goods_price,goods_name,photo,user_id,store_id,auto) VALUES";
